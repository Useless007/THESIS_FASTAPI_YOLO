...
def process_rtsp(cam_ip, save_annotated=True):
    ...
    results = model.predict(source=cam_ip, conf=0.1, iou=0.45, stream=False, device=device)
    

    # Load the image for drawing

    image = cv2.imread(cam_ip)
    ...
    if save_annotated:
        ...
        output_path = cam_ip.replace('.', '_annotated.')
...
if __name__ == "__main__":
    ...
    try:
        ...
        detections, annotated_path = process_rtsp(image_path)
